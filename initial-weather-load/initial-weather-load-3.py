...
try:
    ...
    try:
        ...
        stations_batch_3 = stations[1800:]
        print(f"batch 3: processing stations 1800-{len(stations)-1}:  ({len(stations_batch_3)} stations)")
    ...
    for idx, station in enumerate(stations_batch_3):
        station_name = station['station_name']
        lat = station['lat']
        lon = station['lon']
        
        if (idx + 1) % 100 == 0:
            print(f"processing station {idx + 1}/{len(stations_batch_3)}")
    ...
    try:
        output_path = "s3://mav-delays-weather-slucrx/raw-data/weather/initial-load/batch-3"
